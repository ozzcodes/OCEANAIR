...
all_data = pd.read_csv('../data/Business_ReviewData_2020v2.csv')
labels = ['DIVISION', 'PROFIT']
df = pd.DataFrame(all_data, columns=labels)
grouped = df.groupby('DIVISION')
grouped.boxplot(rot=90, fontsize=14, figsize=(5, 3))

grouped.boxplot(subplots=False, rot=90, fontsize=14)

"""
fig, (ax1, ax2) = plt.subplots(nrows=1, ncols=2, figzie=(5, 3))
